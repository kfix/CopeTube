...
export class CopedJoint {
	...
	develop_coped_point(x, y, offset=0) {
		// transpose any given x (??) & y (radius) point of the cut-tube to its mitered (by this.angle) x's position
	...
	*gen_cope_plot(hflip=false, horiz=false, contra=false, resolution=0.01) {
		...
		while (!(info = plotter.next()).done) {
			...
			var xcut = this.develop_coped_point(x, y, this.offset); //notch x to fit the join_tube angling into the cut_tube
			if (horiz) {
