	...
	*gen_edge_plot(resolution=0.01, contra=false, inner=false) {
		...
		if (this.faces == 0) { //round tube
			...
			while (!(info = stepper.next()).done) {
				...
				yield [x, y, distance, azimuth]; // stream out the cartesian & polar coords
			}
...
export class JointPlot {
	constructor(joint, {hflip=false, vflip=false, horiz=false, contra=false, resolution=0.01, inner=false}={}) {
		...
		while (!(info = this.joint.gen_z_offsets(plotter, roots).next()).done) {
			...
			let z = info.value[1];
			if (!info.value[2]) {
				if (z < this.min || this.min < 0) { this.min = z; }
	...
	*gen_svg_path(offsets) {
		...
		var split_path = false;
		var jog_to_edge = false;
		...
		for (let [d, z, negative] of offsets) {
			...
			if (negative) {
				...
				yield [(this.horiz ? 'V' : 'H') + ' ' + this.max]; //rehome to edge
				jog_to_edge = false;
			}
			...
			if (this.horiz) {
				yield [d, this.cutIn(z)];
			} else {
				yield [this.cutIn(z), d];
			}
		} // offsets
