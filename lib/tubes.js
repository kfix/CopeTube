	...
	*gen_edge_plot(resolution=0.01, contra=false, inner=false) {
		...
		if (this.faces == 0) { //round tube
			...
			while (!(info = stepper.next()).done) {
				...
				// if inner, need to scale up to to fit outer's projection
				...
				var x = ((inner) ? this.inner_radius : this.radius ) * Math.cos(arc_angle)
				var y = ((inner) ? this.inner_radius : this.radius) * Math.sin(arc_angle)
...
export class CopedJoint {
	...
	develop_coped_point(x, y) {
		...
		//   "implicit curve" "envelope" "implicit space curve"
