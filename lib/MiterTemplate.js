...
export default {
	...
	computed: {
		...
		innerAspectRatio() { return (this.southpaw ? "xMaxYMin" : "xMinYMin") + " slice"; },
		innerPathX() {
			return (!this.southpaw) ? this.fromLeft :
				this.fromLeft + this.plot.width - this.innerPlot.width;
		},
	},
	template:
	`<svg class='miterTemplate'
		...
		<svg v-if="cutGauge" class="cope_vbox" v-bind:x="unity(innerPathX)" v-bind:y="unity(fromTop)"
			v-bind:width="unity(innerPathWidth)" v-bind:height="unity(pathHeight)"
			v-bind:preserveAspectRatio="innerAspectRatio" v-bind:viewBox="'0 0 '+(innerPathWidth)+' '+innerPathHeight">
