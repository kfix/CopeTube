...
export default {
	...
	`<svg class='miterTemplate' v-bind:width="unity(width)" v-bind:height="unity(height)"
		...
		<desc></desc>
		<metadata></metadata>
		<svg class="gauge" v-bind:x="unity(width)" y="0"
			v-bind:height="(units == 'in') ? '1in' : '1cm'" width="20px">
			<rect class="previewGauge" x=1px width=100% height=100%
				title="ruler to confirm pixel density" />
			<text class="join_od" dx=12 dy=30>{{(units == 'in') ? '1in' : '1cm'}}</text>
		</svg>
