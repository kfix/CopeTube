...
export default {
	...
	computed: {
		...
		path() { try { return [...this.joint.gen_cope_plot({hflip:this.southpaw})]; } catch { return []; } }, //southpaw:true to invert all graphics
		/* can also be cached as a `$: { ... }` group in Svelte */

		innerTube() { return new tubes.TubeProfile(this.cutOD - (this.cutGauge*2), 0, 0) },
	    innerJoint() { try { return new tubes.CopedJoint(this.innerTube, this.joinTube, this.tubeAngles, this.offset); } catch { }; },
		innerPathSize() { try { return [...this.innerJoint.cope_plot_size()]; } catch { return [0,0]; } },
		innerPathWidth() { return this.innerPathSize.first() },
		innerPathHeight() { return this.innerPathSize.last() },
		innerPath() { try { return [...this.innerJoint.gen_cope_plot({hflip:this.southpaw})]; } catch { return []; } }
	},
	template:
	`<svg class='miterTemplate'
		...
		<svg class="cope_vbox" v-bind:x="unity(fromLeft)" v-bind:y="unity(fromTop)"
			...
			<polygon class="cope" style="fill:none; stroke-dasharray:7 3;" v-bind:points="'0,0 '+innerPath.join(' ')+' 0,'+innerPathHeight" />
		</svg>
