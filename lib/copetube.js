...
export default Vue.component('app', {
	data() {
		return {
			...
			units: "in",
			unitSymbol: UNITS["in"].unitSymbol,
			cutColor: "white",
			keepColor: "lightblue"
		};
	...
	template: `
		<div id='CopeTubeDemo'>
			<form id='controls'>
				...
				<div v-for="unit in UNITS"><input type="radio" v-model="units" :value="unit.value" /></div>
				<label><input type="text" v-model="units" /></label>
				<br />

				<label>layout</label><select v-model="layout">
					<option v-for="layout in LAYOUTS" v-bind:value="layout.value" />
				</select>
				<br />
