...
import JointModel from './JointModel.js';
import MiterTemplate from './MiterTemplate.js';
...
export default Vue.component('app', {
    ...
    methods: {
		...
		printTemplate() {
			var ptwin = window.open('', 'Print template');
	...
	template: `
		<div id='CopeTubeDemo'>
			<div id='template'>
				<miter-template v-bind="{...miter, ...currentUnit(), ...currentLayout()}" />
			</div>
			<div id='visualization'>
				<joint-model v-bind="{...miter, ...currentUnit(), ...currentLayout()}" />
			</div>
			<div id='controls'>
				<form>
					...
					<label>⌀</label>
						<input type="number" step="0.1" inputmode="numeric" v-model="miter.joinOD" />
					...
					<label>⌀</label>
						<input type="number" step="0.1" inputmode="numeric" v-model="miter.cutOD" />
					<label>{{unitSymbol}}⋎</label>
					<br />

					<label>⟀</label>
						<input type="range" min="0" max="44" step="0.5" v-model="miter.angle" />
						<input type="number" min="0" step="any" inputmode="numeric" v-model="miter.angle" />
					<label>°</label>
					<br />

					<label>offset:  </label>
						<input type="number" v-model="miter.offset" />
					<label>{{unitSymbol}}</label>
					<br />
