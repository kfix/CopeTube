...
//import {'update'} as ReactAddons from 'react/dist/react-with-addons.js'; // each will get their own npm evntually
...
// all SVG support could be moving to react-addons: https://github.com/facebook/react/issues/1657
// use https://github.com/reactjs/react-art/blob/master/examples/vector-widget/VectorWidget.js ??
...
/*
function changeByArray(hash, array, newValue, idx) {
    var obj = hash;
    i = 0;
...
// http://blog.sendsonar.com/2015/08/04/angular-like-deep-path-data-bindings-in-react/
var MagicInput = React.createClass({
  onChange: function(e) {
    var state, value;
    state = this.props.binding[0].state;
    changeByArray(state, this.path, e.target.value);
    if (this.props.onChange) {
      this.props.onChange(e);
    }
    return this.props.binding[0].setState(state);
  },
  getValue: function() {
    var i, value;
    value = this.props.binding[0].state;
    this.path || (this.path  = this.props.binding[1].split('.'));
    i = 0;
...
export class CopeTubeDemo extends React.Component {
	...
	// valueLink={}s going away: https://github.com/facebook/react/issues/2302
	linkState(key) {
	...
	render() {
		...
		var { miter, ...other } = this.state;
		return <div id='CopeTubeDemo'>
			<div id='template'>
				<MiterTemplate {...miter} {...other} {...LAYOUTS[this.state.layout][this.state.units]} {...UNITS[this.state.units]} />
			</div>
			<div id='visualization'>
				<JointModel {...miter} {...other} {...LAYOUTS[this.state.layout][this.state.units]} {...UNITS[this.state.units]} />
			</div>
			<form id='controls'>
				<label>⦦</label><input type="text" valueLink={this.linkSubState('miter','joinTitle')} />
				<label>⌀</label><input type="number" valueLink={this.linkSubState('miter','joinOD')} /><label>{UNITS[this.state.units].unitSymbol}◎</label><br />
				<label>⦣⦢</label><input type="text" valueLink={this.linkSubState('miter','cutTitle')} />
